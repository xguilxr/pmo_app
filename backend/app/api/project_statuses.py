...
def list_statuses(
    ...
    return (
        ...
        .filter(ProjectStatus.project_id == project_id, ProjectStatus.deleted_at.is_(None))
...
def create_status(data: StatusCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    s = ProjectStatus(created_by_id=current_user.id, **data.model_dump())
    db.add(s)
    db.commit()
    db.refresh(s)
    return s
